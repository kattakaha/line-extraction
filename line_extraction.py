...
def validation_check(input_file_path, lower_limit, upper_limit):
    ...
    if not os.path.exists(input_file_path):
        raise ValidationError("指定されたファイルがinputディレクトリに存在しません")
...
def main():
    args = sys.argv
    if len(args) != 4:
        raise InputError("コマンドライン引数が無効です")

    # 引数の受け取り
    input_dir_path = os.path.join(os.path.dirname(__file__), "input")
    input_file_path = os.path.join(input_dir_path, args[1])
